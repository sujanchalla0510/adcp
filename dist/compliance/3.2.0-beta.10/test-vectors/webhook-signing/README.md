...
**Canonical URLs.** These vectors are served at `https://adcontextprotocol.org/compliance/{version}/test-vectors/webhook-signing/`, with `{version}` being either a specific immutable release (e.g. `3.0.0`) or `latest` (the moving development snapshot, which can be ahead of GA). Tree preserved — `keys.json`, `negative/*.json`, and `positive/*.json` are all resolvable. SDKs SHOULD pin a released version path and record it rather than requiring a checkout of the spec repo; use `latest` only for development testing. Example: `https://adcontextprotocol.org/compliance/3.0.26/test-vectors/webhook-signing/positive/001-basic-post.json`.
